...
import { AlarmClock, CloudHail, Cloudy, Compass, Droplet, Flame, Sun, Thermometer, Wind } from "lucide-react"
    ...
    <div className="w-full bg-gray-500 p-4 md:p-20">
      <div className="flex min-h-screen flex-col text-black md:flex-row">
        <div className="rounded-t-xl bg-white p-8 md:w-[25%] md:rounded-l-xl md:rounded-tr-none">
          <h1 className="mb-4 text-6xl"><p>{currObj.temp_c}<sup>&deg;C</sup></p>
          </h1>
          <p>{today.day},<span className="text-neutral-500">{today.time} {currObj.is_day === 1 ? "AM" : "PM"}</span></p>
        </div>
        <div className="rounded-b-xl bg-gray-200 p-8 md:w-[75%] md:rounded-r-xl md:rounded-bl-none">
          ...
          <p className="mb-5">Check out today's weather information (last updated {updated.time} {currObj.is_day === 1 ? "AM" : "PM"})</p>
          <Tabs defaultValue="Today" className="w-full gap-10">
            ...
            <TabsContent value="Today">
              <div className="grid w-full grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                <Card className="min-h-35 bg-white px-4 py-2 text-black">
                  ...
                  <h1 className="bold text-3xl ">{currObj.wind_kph} <span className="text-base">Kph</span></h1>
                  <p className="flex gap-2"><Compass />{currObj.wind_dir}</p>
                </Card>
                <Card className="min-h-35 bg-white px-4 py-2 text-black ">
                  <p className="flex items-center justify-between text-neutral-500">
                    Heat index
                ...
                <Card className="min-h-35 bg-white px-4 py-2 text-black">
                  ...
                  <p className="flex gap-2">{currObj.is_day ? "" : "No uv at night"}</p>
                </Card>
                <Card className="min-h-35 bg-white px-4 py-2 text-black">
                  <p className="flex items-center justify-between text-neutral-500">
                    Humidity
                  ...
                  <p className="flex gap-2">{currObj.humidity > 70 ? "Mostly humid" : "Partially humid"}</p>
                </Card>
                <Card className="min-h-35 bg-white px-4 py-2 text-black">
                  <p className="flex items-center justify-between text-neutral-500">
                    Cloud
                  ...
                  <p className="flex gap-2">{currObj.cloud > 70 ? "Mostly cloudy" : "Partially cloudy"}</p>
                </Card>
                <Card className="min-h-35 bg-white px-4 py-2 text-black">
                  <p className="flex items-center justify-between text-neutral-500">
                    Air quality
                  ...
                  <p className="flex gap-2">EPA Index: {currObj.air_quality["us-epa-index"]}</p>
                </Card>
              </div></TabsContent>
            <TabsContent value="Forecast">
              ...
              <div className="w-full">
                <div>
                  <p>Showing Data for {forecastDay.day} ({forecastInfo[0].date})</p>
                  <Card className="flex items-center justify-center bg-white font-semibold text-black">
                    <p>Maximun Tempreture : {forecastInfo[0].day.maxtemp_c}<sup>&deg;C</sup></p>
                    <p>Minimum Tempreture : {forecastInfo[0].day.mintemp_c}<sup>&deg;C</sup></p>
                    <p>Maximun Wind Speed : {forecastInfo[0].day.maxwind_kph} kph</p>
                  ...
                  <div className="flex w-full flex-row gap-4 overflow-x-auto p-5">
                    {
                      hourInfo.map((item, index) => (
                        ...
                        <Card key={index} className="min-h-35 min-w-70 bg-white px-4 py-2 font-semibold text-black *:flex *:items-center *:gap-2">
                          <p><AlarmClock />Time: {getDayAndTime(item.time).time}</p>
                          <p><Thermometer /><span>Tempreture: {item.temp_c}<sup>&deg;C</sup></span></p>
                          <p><Wind /> Wind Speed: {item.wind_kph} kph</p>
                          <p><Compass /> Wind Direction: {item.wind_dir}</p>
                          <p><Droplet /> Humidity: {item.humidity}%</p>
                          <p><CloudHail />Chance of Rain: {item.chance_of_rain}%</p>
                          <p><CloudHail />Will it Rain: {item.will_it_rain ? "Yes" : "No"}</p>
                        </Card>
                      ))
                    }
                  </div>
